...
export function PaymentSettingsManager() {
  ...
  useEffect(() => {
    if (isAdmin) loadRestaurants();
  }, [isAdmin]);

  useEffect(() => {
    if (selectedRestaurant) loadAccountStatus(selectedRestaurant);
  }, [selectedRestaurant]);
  ...
  const loadRestaurants = async () => {
    try {
      const { data, error } = await supabase
        .from("profiles")
        .select("id, restaurant_name, asaas_account_status");
      if (error) throw error;
      setRestaurants(data || []);
      if (data && data.length > 0) setSelectedRestaurant(data[0].id);
    } catch {
  ...
  const loadAccountStatus = async (restaurantId: string) => {
    try {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) return;

      const { data, error } = await supabase.functions.invoke("asaas-account", {
        body: { action: "get-status", restaurantId },
      });
      if (error) throw error;
  ...
  const handleRefreshStatus = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("asaas-account", {
        body: { action: "refresh-status", restaurantId: selectedRestaurant },
      });
      if (error) throw error;
      if (data?.status) {
    ...
    return (
      <Card>
        <CardContent className="py-8 text-center">
          <p className="text-muted-foreground">Apenas administradores podem gerenciar pagamentos.</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Ative e gerencie recebimentos online dos restaurantes via Asaas</CardDescription>
        </CardHeader>
          ...
          <CardContent className="space-y-4">
                ...
                {!showForm ? (
                  <div className="text-center py-6 space-y-3">
                    <p className="text-muted-foreground">
                      Este restaurante ainda não possui conta de recebimentos ativa.
                    </p>
              ...
              <div className="space-y-4">
                <div className="p-4 bg-yellow-500/5 border border-yellow-500/20 rounded-lg space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    O restaurante precisa completar o cadastro financeiro para começar a receber pagamentos.
                  </p>
                </div>
            ...
            {accountStatus?.status === "ativa" && (
              <div className="p-4 bg-green-500/5 border border-green-500/20 rounded-lg space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Este restaurante pode receber pagamentos via PIX e cartão de crédito.
                </p>
                {accountStatus.createdAt && (
                  <p className="text-xs text-muted-foreground">
                    Ativada em: {new Date(accountStatus.createdAt).toLocaleDateString("pt-BR")}
                  </p>
                )}
              </div>
            )}
